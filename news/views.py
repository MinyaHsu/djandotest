...
def newsindex(request, pageindex=None):  #首頁
	global page1
	pagesize = 8
...
def newsdetail(request, detailid=None):  #詳細頁面
...
def newslogout(request):  #登出
	auth.logout(request)
	return redirect('/newsindex/')

def newsadminmain(request, pageindex=None):  #管理頁面
	global page1
	pagesize = 8
	newsall = models.NewsUnit.objects.all().order_by('-id')
	datasize = len(newsall)
